...
internal fun JewelTitleBar(
    setup: DesktopTitleBar.Setup,
    actions: List<DesktopTitleBar.TitleAction>,
    menubar: @Composable () -> Unit = {},
) {
    val prefs = DesktopAppSetup.get().prefs
    val scope = rememberCoroutineScope()
    val theme by DesktopPrefs.get().jewelTheme.collectAsStateNotNull()
    ...
    with(LocalDecoratedWindowScope.current) {
                ...
                if (setup.showThemeSelector) {
                    ...
                    TitleIconButton(theme.imageVector, text) {
                        scope.launch(Dispatchers.IO) {
                            DesktopPrefs.get().jewelTheme.update(nextTheme)
                        }
                    }
                }
